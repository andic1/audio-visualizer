...
type LabMode = "classic" | "rhythm" | "mood";
...
interface IAppState {
  ...
  actions: {
    ...
    setLabMode: (newMode: LabMode) => void;
...
const useAppState = create<IAppState>((set) => ({
  ...
  actions: {
    ...
    setAudio: (newAudio: Partial<IAudioState>) =>
      set((state) => {
        ...
        const isFileSelected =
          nextAudio.source === "FILE_UPLOAD" && nextAudio.file instanceof File;

        return {
          audio: nextAudio,
          ...(isFileSelected
            ? {
                // Force-enable audio mode so playback + analyzer mounts.
                mode: APPLICATION_MODE.AUDIO,
                // Keep classic look by default if user is in classic lab mode.
                visual:
                  state.labMode === "classic" ? VISUAL_REGISTRY.grid : state.visual,
                mappers: {
                  ...state.mappers,
                  energyTracker: new EnergyTracker(0),
                },
                appearance: {
                  ...state.appearance,
                  // In audio mode we allow energy-driven updates.
    ...
    setLabMode: (newMode: LabMode) =>
      set((state) => {
        const hasLocalFile =
          state.audio.source === "FILE_UPLOAD" && state.audio.file instanceof File;

        // If a local file is loaded, keep application mode in AUDIO so the
        // AudioAnalyzer (and its underlying HTMLAudioElement) stays mounted.
        const nextMode = hasLocalFile ? APPLICATION_MODE.AUDIO : undefined;
        ...
        switch (newMode) {
          case "rhythm":
            return {
              labMode: newMode,
              ...(nextMode ? { mode: nextMode } : { mode: APPLICATION_MODE.AUDIO }),
              visual: VISUAL_REGISTRY.swarm,
              analyzers: {
                ...state.analyzers,
                fft: {
                  ...state.analyzers.fft,
              ...
              mappers: {
                ...state.mappers,
                // 更“激烈”的流体运动
                motionMapper: new MotionMapper_Noise(2.6, 1.05),
              },
            };
          case "mood":
            return {
              labMode: newMode,
              ...(nextMode ? { mode: nextMode } : { mode: APPLICATION_MODE.AUDIO }),
              visual: VISUAL_REGISTRY.ribbons,
              analyzers: {
                ...state.analyzers,
                fft: {
                  ...state.analyzers.fft,
                  energyMeasure: "overall",
                },
              },
              appearance: {
                ...state.appearance,
                palette: COLOR_PALETTE.SEASIDE,
                colorBackground: true,
                paletteTrackEnergy: false,
              },
              mappers: {
                ...state.mappers,
                // 更“柔和/漂浮”的运动
                motionMapper: new MotionMapper_Noise(0.9, 0.28),
              },
            };
          case "classic":
          default:
...
export const useLabMode = () => useAppState((state) => state.labMode);
